...
public class _4_TwoEqualSumInAnArray {
    ...
    public static void main(String[] args) {
        ...
        if(sum % 2 == 0) {
            // using recursion.
            System.out.println(
                    _3_SumOfTwo_or_N_nt_IsGivenNumber.sumOf_N_NumbersIsEqualToGivenValueRecursion(nums, nums.length, sum/2));

            // using DP.
            boolean[][] memory = new boolean[nums.length + 1][sum/2 + 1];
            System.out.println(
                    _3_SumOfTwo_or_N_nt_IsGivenNumber.sumOf_N_NumbersIsEqualToGivenValueDP(nums, nums.length, sum/2, memory));
